use crate::*;
use std::{fs::{OpenOptions}, io::{BufRead, BufReader, BufWriter, Write}, path::{Path, PathBuf}, process::Command};
...
impl Git{
    ...
    pub fn new() -> Result<Self> {
        ...
        while Repository::open(repo_dir.clone()).is_err() {
            if repo_dir.pop() == false {
                return Err(err_msg("not a git repository (or any of the parent directories)"))
            }
        }
        Ok(Self{
    ...
    pub fn get_uncommitted_files(&self) -> Result<Vec<String>> {
        if self.repo.state() != RepositoryState::Clean {
            ...
            for stat in statuses.iter() {
                res.push(stat.path().expect("Unable convert file path to str").to_string());
    ...
    pub fn get_local_ignore(&self) -> Result<Vec<String>> {
        let exclude = OpenOptions::new().read(true).create(false)
    ...
    pub fn update_local_ignore(&self, paths: Vec<String>) -> Result<()> {
        ...
        let exclude = OpenOptions::new().write(true).create(true).truncate(true)
                                .open(self.repo.path().join("info/exclude"))?;
        let mut exclude = BufWriter::new(exclude);
        let mut index = self.repo.index()?;

        for path in paths {
            if let Some(c) = path.trim().chars().nth(0) {
                if c != '#' {
